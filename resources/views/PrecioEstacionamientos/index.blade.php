...
<section class="col-lg-12 connectedSortable">
	<div class="box box-warning">
		<div class="box-header with-border">
			<h3 class="box-descripcion">Lista de PrecioEstacionamientos</h3>
		</div>          
		<div class="box-body">
	   		<div class="">
			<div class="panel panel-default">
				<div class="panel-heading">
					<ul>
						...
						@can('PrecioEstacionamientos Add')
							<a href="#" id="massadd-modal" class="massmodal"><li>Añadir un PrecioEstacionamientos</li></a>
						@endcan
					</ul>
				</div>
				...
				<div class="panel-body" style="overflow-x:auto;">
					...
					<tr id="item_{{$lists->id}}"" class="item{{$lists->id}} @if($lists->is_published) warning @endif">
						...
						<td class="col1">{{ $lists->precio }}</td>
						<td class="col1">{{ $lists->tiempo }}</td>
						<td class="col1">
						<script type="text/javascript">
						...
						<td>
						@can('PrecioEstacionamientos Show')
						<button class="massshow-modal btn btn-success" 
						data-id="{{ $lists->id}}"
						data-nombre="{{ $lists->nombre}}"
						data-precio="{{ $lists->precio}}"
						data-tiempo="{{ $lists->tiempo}}"
						...
						@endcan		
						@can('PrecioEstacionamientos Editar')
						...
						@endcan
						@can('PrecioEstacionamientos Eliminar') 
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" id="formmass" role="form">
						...
						<div class='form-group' id='precio' >
							<label class='control-label col-sm-2' for='descripcion'>precio:</label>
							<div class='col-sm-10'>
								<input type='text' class='form-control' id='precio_mass' maxlength='45'   required='required' autofocus>
								<p class='errorprecio text-center alert alert-danger hidden'></p>
							</div>
						</div>
						
						<div class='form-group' id='tiempo' >
							<label class='control-label col-sm-2' for='descripcion'>tiempo:</label>
							<div class='col-sm-10'>
								<input type='text' class='form-control' id='tiempo_mass' maxlength='45'   required='required' autofocus>
								<p class='errortiempo text-center alert alert-danger hidden'></p>
							</div>
						</div>
						
						<div class="form-group">
							<label class="control-label col-sm-2" for="descripcion">tipo_vehiculo_id:</label>
							<div class="col-sm-10">
...
<script type='text/javascript'>
	...
	<script type='text/javascript'>
		...
		$(document).on('click', '.massshow-modal', function() {
			...
			$('#precio_mass').val($(this).data('precio'));
			$('#tiempo_mass').val($(this).data('tiempo'));
		...
		$('.modal-footer').on('click', '.add', function() {
			...
			$.ajax({
				type: 'POST',
				url: 'PrecioEstacionamientos',
				data: {

				'id': $('#id_mass').val(),
				'nombre': $('#nombre_mass').val(),
				'precio': $('#precio_mass').val(),
				'tiempo': $('#tiempo_mass').val(),
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.errors){
                	...
                	toastr.success('Modificación Exitosa PrecioEstacionamientos!', 'Datos Modificados', {timeOut: 5000});
...
function verificar(data) {
	...
	if (data.errors.tiempo) {
    	$(".errortiempo").removeClass("hidden");
    	$(".errortiempo").text(data.errors.tiempo);
    }

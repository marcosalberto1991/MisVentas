...
<script type='text/javascript'>
			<?php echo "const  Forasitios_parqueadero_id= $sitios_parqueadero_id;"; ?>
...
<section class="col-lg-12 connectedSortable">
	<div class="box box-warning">
		<div class="box-header with-border">
			<h3 class="box-descripcion">Lista de Zona de Parqueadero</h3>
		</div>          
		<div class="box-body">
	   		<div class="">
			<div class="panel panel-default">
				<div class="panel-heading">
					<ul>
						...
						@can('ZonaParqueadero Add')
							<a href="#" id="massadd-modal" class="massmodal"><li>Añadir una Zona de Parqueadero</li></a>
						@endcan
					</ul>
				</div>
				...
				<div class="panel-body" style="overflow-x:auto;">
					...
					<tr id="item_{{$lists->id}}"" class="item{{$lists->id}} @if($lists->is_published) warning @endif">
						...
						<td class="col1">
						<script type="text/javascript">
							resulmunicipios_id=Forasitios_parqueadero_id.find( cas => cas.id == {{ $lists->sitios_parqueadero_id }} );
						...
						<td>
						<a class="massshow-modal btn btn-success"
						href="{{ action('NivelesController@lista',['id' => $lists->id]) }}" 
						>
						<span class="glyphicon glyphicon-eye-open"></span> Ver los niveles de parqueadero</a>	

							
						@can('ZonaParqueadero Show')
						<button class="massshow-modal btn btn-success" 
						data-id="{{ $lists->id}}"
						data-nombre="{{ $lists->nombre}}"
						data-sitios_parqueadero_id="{{ $lists->sitios_parqueadero_id}}"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" id="formmass" role="form">
						...
						<div class="form-group">
							<label class="control-label col-sm-2" for="descripcion">Sitios de parqueadero:</label>
							<div class="col-sm-10">
								
								<select class="form-control" id="sitios_parqueadero_id_mass" required="required" autofocus >
 									<option value=""></option>

 								@foreach($sitios_parqueadero_id as $lists)
  									<option value="{{$lists->id}}">{{$lists->nombre}}</option>
								@endforeach
								
								</select>
								<!--
								<input type="select" class="form-control" id=sitios_parqueadero_id_mass" required="required" autofocus>
								-->
								<p class="errorsitios_parqueadero_id text-center alert alert-danger hidden"> llenar los datos</p>
							</div>
						</div>
...
<script type='text/javascript'>
	...
	<script type='text/javascript'>
		...
		$(document).on('click', '.massshow-modal', function() {
			...
			$('#sitios_parqueadero_id_mass').val($(this).data('sitios_parqueadero_id'));
		...
		$('.modal-footer').on('click', '.add', function() {
			...
			$.ajax({
				...
				'sitios_parqueadero_id': $('#sitios_parqueadero_id_mass').val(),
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.errors){
                	...
                	toastr.success('Modificación Exitosa ZonaParqueadero!', 'Datos Modificados', {timeOut: 5000});
...
function verificar(data) {
	...
	$('.errorsitios_parqueadero_id').addClass('hidden');
	...
	if (data.errors.sitios_parqueadero_id) {
    	$(".errorsitios_parqueadero_id").removeClass("hidden");
    	$(".errorsitios_parqueadero_id").text(data.errors.sitios_parqueadero_id);
    }
...
<script type='text/javascript'>
	...
	const resulsitios_parqueadero_id=Forasitios_parqueadero_id.find( cas => cas.id == data.sitios_parqueadero_id ); 
	...
	var tabla=
		...
		"<td>"+ data.nombre+"</td>"+
		"<td>"+ resulsitios_parqueadero_id["nombre"]  +"</td>"+
	...
	@can('ZonaParqueadero Show') 
		...
		"data-sitios_parqueadero_id='"+ data.sitios_parqueadero_id+"'"+

...
<section class="col-lg-12 connectedSortable">
	<div class="box box-warning">
		<div class="box-header with-border">
			<h3 class="box-descripcion">Lista de Categoría de los Arboles</h3>
		</div>          
		<div class="box-body">
	   		<div class="">
			<div class="panel panel-default">
				<div class="panel-heading">
					<ul>
						...
						@can('Categoria_Arbol Add')
							<a href="#" id="massadd-modal" class="massmodal"><li>Añadir una Categoría</li></a>
						@endcan
					</ul>
				</div>
				...
				<div class="panel-body" style="overflow-x:auto;">
					...
					<tr id="item_{{$lists->id}}"" class="item{{$lists->id}} @if($lists->is_published) warning @endif">
						...
						<td class="col1">{{ $lists->altura_max }}</td>
						<td class="col1">{{ $lists->altura_min }}</td>
						...
						<td>
						@can('Categoria_Arbol Show')
						<button class="massshow-modal btn btn-success" 
						data-id="{{ $lists->id}}"
						data-nombre="{{ $lists->nombre}}"
						data-altura_max="{{ $lists->altura_max}}"
						data-altura_min="{{ $lists->altura_min}}"
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" id="formmass" role="form">
						...
						<div class='form-group' id='nombre' >
							<label class='control-label col-sm-2' for='descripcion'>Tipo:</label>
							<div class='col-sm-10'>
						...
						<div class='form-group' id='altura_max' >
							<label class='control-label col-sm-2' for='descripcion'>Altura máxima:</label>
							<div class='col-sm-10'>
								<input type='text' class='form-control input-number-coma-punto' id='altura_max_mass' maxlength='112'   required='required' autofocus>
								<p class='erroraltura_max text-center alert alert-danger hidden'></p>
							</div>
						</div>
						
						<div class='form-group' id='altura_min' >
							<label class='control-label col-sm-2' for='descripcion'>Altura mínima:</label>
							<div class='col-sm-10'>
								<input type='text' class='form-control input-number-coma-punto' id='altura_min_mass' maxlength='112'   required='required' autofocus>
								<p class='erroraltura_min text-center alert alert-danger hidden'></p>
							</div>
						</div>
...
<script type='text/javascript'>
	...
	<script type='text/javascript'>
		...
		$(document).on('click', '.massshow-modal', function() {
			...
			$('#altura_max_mass').val($(this).data('altura_max'));
			$('#altura_min_mass').val($(this).data('altura_min'));
		...
		$('.modal-footer').on('click', '.add', function() {
			$.ajax({
				...
				'altura_max': $('#altura_max_mass').val(),
				'altura_min': $('#altura_min_mass').val(),
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.errors){
                	...
                	toastr.success('Modificación Exitosa Categoria_Arbol!', 'Datos Modificados', {timeOut: 5000});
...
function verificar(data) {
	...
	$('.erroraltura_max').addClass('hidden');
	$('.erroraltura_min').addClass('hidden');
	...
	if (data.errors.altura_max) {
    	$(".erroraltura_max").removeClass("hidden");
    	$(".erroraltura_max").text(data.errors.altura_max);
    }
    
	if (data.errors.altura_min) {
    	$(".erroraltura_min").removeClass("hidden");
    	$(".erroraltura_min").text(data.errors.altura_min);
    }
...
<script type='text/javascript'>
	...
	var tabla=
		...
		"<td>"+ data.nombre+"</td>"+
		"<td>"+ data.altura_max+"</td>"+
		"<td>"+ data.altura_min+"</td>"+
	...
	@can('Categoria_Arbol Show') 
		...
		"data-altura_max='"+ data.altura_max+"'"+
		"data-altura_min='"+ data.altura_min+"'"+

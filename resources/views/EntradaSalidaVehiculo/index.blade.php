...
<script type='text/javascript'>
			<?php echo "const  Foracategoria_parqueadero_id= $categoria_parqueadero_id;"; ?>
			<?php echo "const  Foratipo_vehiculo_id= $tipo_vehiculo_id;"; ?>
...
<section class="col-lg-12 connectedSortable">
	<div class="box box-warning">
		<div class="box-header with-border">
			<h3 class="box-descripcion">Lista de Parqueadero por Vehiculo</h3>
		</div>          
		<div class="box-body">
		   	...
		   	<div class="">
				<div class="panel panel-default">
					<div class="panel-heading">
						<ul>
							<li><i class="fa fa-file-text-o"></i> Tipo de vehiculo</li>
							
						</ul>
					</div>
					<div class="panel-body">
						@foreach($tipo_vehiculo_id as $tipo_vehiculo)
						<div class="jumbotron col-sm-3">
  							<div class="container">
						{!! Form::open(['method' => 'POST', 'route' => ['EntradaSalidaVehiculo.store'],'target'=>"_blank",'onclick'=>"setTimeout('actualiza()',1000);" ]) !!}
								<h3>{{$tipo_vehiculo->nombre }}</h3>

						<input type='hidden' value="{{$tipo_vehiculo->id }}" name="tipo_vehiculo_id" class='form-control'  required='required' autofocus>
...
<section class="col-lg-12 connectedSortable">
	<div class="box box-warning">
		<div class="box-header with-border">
			<h3 class="box-descripcion">Lista de ParqueaderoVehiculo</h3>
		</div>          
		<div class="box-body">
	   		<div class="">
			<div class="panel panel-default">
				...
				<div class="panel-body" style="overflow-x:auto;">
					...
					<tr>
						...
						<th>Ultima Modificación</th><th>Acción</th>
					...
					@foreach($listmysql as $registro)


											  
					<tr id="item_{{$registro->id}}"" class="item{{$registro->id}}">
						@if($registro->estado_vehiculo==2)
						<td class="col1">{{ $registro->id }}</td>
						
						<td class="col1"> lists->codigo </td>
						<td class="col1">{{ $registro->fecha_ingreso }}</td>
						<td class="col1">lists->tipo_vehiculo_id_pk->nombre </td>
						<td>{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $registro->updated_at)->diffForHumans() }}</td>
						<td>
							

						@can('ParqueaderoVehiculo cobra')
						{!! Form::open(['method' => 'POST', 'route' => ['EntradaSalidaVehiculo.cobro'],'target'=>"_blank",'onclick'=>"setTimeout('eliminar_lista($registro->id)',3000);" ]) !!}
						<input type='hidden' value="{{$registro->id }}" name="parqueadero_vehiculo_id" class='form-control'  required='required' autofocus>
						{!! Form::submit('Cobra el parqueadero', ['class' => 'btn btn-primary'] ) !!}
    					{!! Form::close() !!}
...
<section class="col-lg-12 connectedSortable">
	<div class="box box-warning">
		<div class="box-header with-border">
			<h3 class="box-descripcion">Lista de Parqueadero Disponibles</h3>
		</div>          
		<div class="box-body">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						@foreach($tipo_vehiculo_id as $tipo_vehiculo)
							...
							<div class="col-sm-3"><h3>{{$tipo_vehiculo->nombre}}</h3>
								@foreach($tipo_vehiculo->parqueadero_vehiculo_all as $parqueadero_vehiculo )
									<ul class="list-group">
										@if($parqueadero_vehiculo->estado_vehiculo==1)
								  		<li class="list-group-item">{{$parqueadero_vehiculo->codigo}}</li>
										@endif
									</ul>
								@endforeach
							</div>
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" id="formmass" role="form">
						...
						<div class='form-group' id='numero' >
							<label class='control-label col-sm-2' for='descripcion'>Numero:</label>
							<div class='col-sm-10'>
								<input type='text' class='form-control' id='numero_mass' maxlength='11'   required='required' autofocus>
						...
						-->
						<div class="form-group">
							<label class="control-label col-sm-2" for="descripcion">Categoría de parqueadero:</label>
							<div class="col-sm-10">
								
								<select class="form-control" id="categoria_parqueadero_id_mass" required="required" autofocus >
 									<option value=""></option>

 								@foreach($categoria_parqueadero_id as $lists)
  									<option value="{{$lists->id}}">{{$lists->nombre}}</option>
								@endforeach
								
								</select>
								<!--
								<input type="select" class="form-control" id=categoria_parqueadero_id_mass" required="required" autofocus>
								-->
								<p class="errorcategoria_parqueadero_id text-center alert alert-danger hidden"> llenar los datos</p>
							</div>
						</div>
						
						<div class="form-group">
							<label class="control-label col-sm-2" for="descripcion">Tipo de vehículo:</label>
							<div class="col-sm-10">
								
								<select class="form-control" id="tipo_vehiculo_id_mass" required="required" autofocus >
 									<option value=""></option>

 								@foreach($tipo_vehiculo_id as $lists)
  									<option value="{{$lists->id}}">{{$lists->nombre}}</option>
								@endforeach
								
								</select>
								<!--
								<input type="select" class="form-control" id=tipo_vehiculo_id_mass" required="required" autofocus>
								-->
								<p class="errortipo_vehiculo_id text-center alert alert-danger hidden"> llenar los datos</p>
							</div>
						</div>
						
						<div class="form-group">
							<label class="control-label col-sm-2" for="descripcion">Estado:</label>
							<div class="col-sm-10">
								
								<select class="form-control" id="estado_id_mass" required="required" autofocus >
 									<option value=""></option>
								...
								-->
								<p class="errorestado_id text-center alert alert-danger hidden"> llenar los datos</p>
							</div>
						</div>
...
function actualiza() {
	...
	<script type='text/javascript'>
		...
		$(document).on('click', '.massshow-modal', function() {
			...
			$('#numero_mass').val($(this).data('numero'));
			$('#codigo_mass').val($(this).data('codigo'));
			$('#categoria_parqueadero_id_mass').val($(this).data('categoria_parqueadero_id'));
			$('#tipo_vehiculo_id_mass').val($(this).data('tipo_vehiculo_id'));
		...
		$('.modal-footer').on('click', '.add', function() {
			...
			$.ajax({
				type: 'POST',
				url: 'ParqueaderoVehiculo',
				data: {

				'id': $('#id_mass').val(),
				'numero': $('#numero_mass').val(),
				'codigo': $('#codigo_mass').val(),
				'categoria_parqueadero_id': $('#categoria_parqueadero_id_mass').val(),
				'tipo_vehiculo_id': $('#tipo_vehiculo_id_mass').val(),
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.errors){
                	...
                	toastr.success('Modificación Exitosa ParqueaderoVehiculo!', 'Datos Modificados', {timeOut: 5000});
...
function verificar(data) {
	...
	$('.errorcategoria_parqueadero_id').addClass('hidden');
	$('.errortipo_vehiculo_id').addClass('hidden');
	...
	if (data.errors.categoria_parqueadero_id) {
    	$(".errorcategoria_parqueadero_id").removeClass("hidden");
    	$(".errorcategoria_parqueadero_id").text(data.errors.categoria_parqueadero_id);
    }
    
	if (data.errors.tipo_vehiculo_id) {
    	$(".errortipo_vehiculo_id").removeClass("hidden");
    	$(".errortipo_vehiculo_id").text(data.errors.tipo_vehiculo_id);
    }
...
<script type='text/javascript'>
	...
	const resulcategoria_parqueadero_id=Foracategoria_parqueadero_id.find( cas => cas.id == data.categoria_parqueadero_id ); 
		const resultipo_vehiculo_id=Foratipo_vehiculo_id.find( cas => cas.id == data.tipo_vehiculo_id ); 
	...
	var tabla=
		...
		"<td>"+ data.numero+"</td>"+
		"<td>"+ data.codigo+"</td>"+
		"<td>"+ resulcategoria_parqueadero_id["nombre"]  +"</td>"+
		"<td>"+ resultipo_vehiculo_id["nombre"]  +"</td>"+
		"<td>"+ resulestado_id["nombre"]  +"</td>"+
		"<td>"+ data.created_at+"</td>"+
		"<td>"+ data.updated_at+"</td>"+
		
		'<td>Ahorra</td><td>'+					
	@can('ParqueaderoVehiculo Show') 
		' <button class="massshow-modal btn btn-success"  ' + 
		"data-id='"+ data.id+"'"+
		"data-numero='"+ data.numero+"'"+
		"data-codigo='"+ data.codigo+"'"+
		"data-categoria_parqueadero_id='"+ data.categoria_parqueadero_id+"'"+
	...
	@can('ParqueaderoVehiculo Editar')
	...
	@can('ParqueaderoVehiculo Eliminar') 

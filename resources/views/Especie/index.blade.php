...
<section class="col-lg-12 connectedSortable">
	 <div class="box box-warning">
			<div class="box-header with-border">
			  <h3 class="box-descripcion">Lista de Especie</h3>
			</div>          
			<!-- /.box-header -->
	  <div class="box-body">
	   <div class="">
		<div class="panel panel-default">
			<div class="panel-heading">
				<ul>
					...
					@can('Especie Add')
					<a href="#" id="massadd-modal" class="massmodal"><li>Añadir un Especie</li></a>
		...
		<td class="col1">{{ $lists->nombre_cientifico }}</td>
		...
		data-nombre_cientifico="{{ $lists->nombre_cientifico}}"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" id="formmass" role="form">
						...
						<div class='form-group'>
							<label class='control-label col-sm-2' for='descripcion'>nombre_cientifico:</label>
							<div class='col-sm-10'>
								<input type='text' class='form-control' id='nombre_cientifico_mass' maxlength='45'  required='required' autofocus>
								<p class='errornombre_cientifico text-center alert alert-danger hidden'></p>
							</div>
						</div>
...
$('#nombre_cientifico_mass').val($(this).data('nombre_cientifico'));
...
;
		...
		$('.modal-footer').on('click', '.add', function() {
			$.ajax({
				...
				'id': $('#id_mass').val(),'nombre': $('#nombre_mass').val(),'nombre_cientifico': $('#nombre_cientifico_mass').val(),'created_at': $('#created_at_mass').val(),'updated_at': $('#updated_at_mass').val(),

					'_token': $('input[name=_token]').val(),
				},
				success: function(data) {
				   
					$('.errorid').addClass('hidden');$('.errornombre').addClass('hidden');$('.errornombre_cientifico').addClass('hidden');$('.errorcreated_at').addClass('hidden');$('.errorupdated_at').addClass('hidden');;
					if ((data.errors)) {
						setTimeout(function () {
							$('#massModal').modal('show');
						...
						$('.errorid').removeClass('hidden');$('.errorid').text(data.errors.id);$('.errornombre').removeClass('hidden');$('.errornombre').text(data.errors.nombre);$('.errornombre_cientifico').removeClass('hidden');$('.errornombre_cientifico').text(data.errors.nombre_cientifico);$('.errorcreated_at').removeClass('hidden');$('.errorcreated_at').text(data.errors.created_at);$('.errorupdated_at').removeClass('hidden');$('.errorupdated_at').text(data.errors.updated_at);;
					} else {
...
//add
			...
			"<td>"+ data.updated_at+"</td>"+
				...
				@can('Especie Show')
						...
						"' data-nombre_cientifico='"+ data.nombre_cientifico+
		...
		$(document).on('click', '.edit-modal', function() {
			...
			'nombre_cientifico': $('#nombre_cientifico_mass').val(),
			...
			$('.errornombre_cientifico').addClass('hidden');
			...
			if ((data.errors)) {
				...
				if (data.errors.nombre_cientifico) {
                	$(".error.nombre_cientifico").removeClass("hidden");
                	$(".error.nombre_cientifico").text(data.errorsnombre_cientifico);
                }
				...
				if (data.errors.updated_at) {
                	...
                	toastr.success('Successfully updated Especie!', 'Success Alert', {timeOut: 5000});
                //update

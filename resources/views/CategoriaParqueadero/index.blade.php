...
<script type='text/javascript'>
			<?php echo "const  Forazona_parqueadero_id= $zona_parqueadero_id;"; ?>
			<?php echo "const  Foraniveles_id= $niveles_id;"; ?>
...
<section class="col-lg-12 connectedSortable">
	<div class="box box-warning">
		<div class="box-header with-border">
			<h3 class="box-descripcion">Lista de Categoria de Parqueadero</h3>
		</div>          
		<div class="box-body">
	   		<div class="">
			<div class="panel panel-default">
				<div class="panel-heading">
					<ul>
						...
						@can('CategoriaParqueadero Add')
							<a href="#" id="massadd-modal" class="massmodal"><li>Añadir un Categoria de Parqueadero</li></a>
						@endcan
					</ul>
				</div>
				...
				<div class="panel-body" style="overflow-x:auto;">
					...
					<tr id="item_{{$lists->id}}"" class="item{{$lists->id}} @if($lists->is_published) warning @endif">
						...
						<td class="col1">{{ $lists->letra }}</td>
						<td class="col1">
						<script type="text/javascript">
							resulmunicipios_id=Forazona_parqueadero_id.find( cas => cas.id == {{ $lists->zona_parqueadero_id }} );
							document.write(resulmunicipios_id.nombre); 
						</script>
						</td>
							<td class="col1">
						<script type="text/javascript">
							resulmunicipios_id=Foraniveles_id.find( cas => cas.id == {{ $lists->niveles_id }} );
							document.write(resulmunicipios_id.nombre); 
						</script>
						</td>
							<td class="col1">
						...
						<td>
						@can('CategoriaParqueadero Show')
						<button class="massshow-modal btn btn-success" 
						data-id="{{ $lists->id}}"
						data-letra="{{ $lists->letra}}"
						data-zona_parqueadero_id="{{ $lists->zona_parqueadero_id}}"
						data-niveles_id="{{ $lists->niveles_id}}"
						...
						@endcan		
						@can('CategoriaParqueadero Editar')
						...
						@endcan
						@can('CategoriaParqueadero Eliminar') 
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" id="formmass" role="form">
						...
						<div class='form-group' id='letra' >
							<label class='control-label col-sm-2' for='descripcion'>Letra:</label>
							<div class='col-sm-10'>
								<input type='text' class='form-control' id='letra_mass' maxlength='45'   required='required' autofocus>
								<p class='errorletra text-center alert alert-danger hidden'></p>
							</div>
						</div>
						
						<div class="form-group">
							<label class="control-label col-sm-2" for="descripcion">Zona de parqueadero:</label>
							<div class="col-sm-10">
								
								<select class="form-control" id="zona_parqueadero_id_mass"  required="required" autofocus >
 									<option value=""></option>

 								@foreach($zona_parqueadero_id as $lists)
  									<option value="{{$lists->id}}">{{$lists->nombre}}</option>
								@endforeach
								
								</select>
								<!--
								<input type="select" class="form-control" id=zona_parqueadero_id_mass" required="required" autofocus>
								-->
								<p class="errorzona_parqueadero_id text-center alert alert-danger hidden"> llenar los datos</p>
							</div>
						</div>
						
						<div class="form-group">
							<label class="control-label col-sm-2" for="descripcion">Niveles:</label>
							<div class="col-sm-10">
								
								<select class="form-control" id="niveles_id_mass" required="required" autofocus >
 									<option value=""></option>
								...
								<!--
								<input type="select" class="form-control" id=niveles_id_mass" required="required" autofocus>
								-->
								<p class="errorniveles_id text-center alert alert-danger hidden"> llenar los datos</p>
							</div>
						</div>
...
<script type='text/javascript'>
	...
	<script type='text/javascript'>
		...
		$(document).on('click', '.massshow-modal', function() {
			...
			$('#letra_mass').val($(this).data('letra'));
			$('#zona_parqueadero_id_mass').val($(this).data('zona_parqueadero_id'));
			$('#niveles_id_mass').val($(this).data('niveles_id'));
		...
		$('.modal-footer').on('click', '.add', function() {
			...
			$.ajax({
				type: 'POST',
				url: 'CategoriaParqueadero',
				data: {

				'id': $('#id_mass').val(),
				'letra': $('#letra_mass').val(),
				'zona_parqueadero_id': $('#zona_parqueadero_id_mass').val(),
				'niveles_id': $('#niveles_id_mass').val(),
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.errors){
                	...
                	toastr.success('Modificación Exitosa CategoriaParqueadero!', 'Datos Modificados', {timeOut: 5000});
...
function verificar(data) {
	...
	$('.errorletra').addClass('hidden');
	$('.errorzona_parqueadero_id').addClass('hidden');
	$('.errorniveles_id').addClass('hidden');
	...
	if (data.errors.letra) {
    	$(".errorletra").removeClass("hidden");
    	$(".errorletra").text(data.errors.letra);
    }
    
	if (data.errors.zona_parqueadero_id) {
    	$(".errorzona_parqueadero_id").removeClass("hidden");
    	$(".errorzona_parqueadero_id").text(data.errors.zona_parqueadero_id);
    }
    
	if (data.errors.niveles_id) {
    	$(".errorniveles_id").removeClass("hidden");
    	$(".errorniveles_id").text(data.errors.niveles_id);
    }
...
<script type='text/javascript'>
	...
	const resulzona_parqueadero_id=Forazona_parqueadero_id.find( cas => cas.id == data.zona_parqueadero_id ); 
		const resulniveles_id=Foraniveles_id.find( cas => cas.id == data.niveles_id ); 
	...
	var tabla=
		...
		"<td>"+ data.letra+"</td>"+
		"<td>"+ resulzona_parqueadero_id["nombre"]  +"</td>"+
		"<td>"+ resulniveles_id["nombre"]  +"</td>"+
	...
	@can('CategoriaParqueadero Show') 
		...
		"data-zona_parqueadero_id='"+ data.zona_parqueadero_id+"'"+
		"data-niveles_id='"+ data.niveles_id+"'"+
	...
	$(document).ready(function(){
        $('#zona_parqueadero_id_mass').on('change',function(){

            var zona_parqueadero_id = $(this).val();
            $.ajax({
                type:"get", 
                url : "CategoriaParqueadero/niveles/"+zona_parqueadero_id,
                data: {
				'_token': $('input[name=_token]').val(),
				},    
                success: function(data) {
                    //$('#niveles_id_mass').append('<option value=""></option>');
                    for (var i = 0; i < data.length; i++) { 
                    	$('#niveles_id_mass').append('<option value="'+data[i].id+'">'+data[i].nombre+'</option>');
                    }
                },
               	error: function(jqXHR, text, error){
            		toastr.error('Error de operación!', 'No se pudo Añadir los datos<br>'+error, {timeOut: 5000});	

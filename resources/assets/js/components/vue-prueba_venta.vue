<template>
<div class="row">
    <div v-if="formulario_prueba_venta" class="col-lg-12 ">
      ...
      <form ref="form"   v-on:submit.prevent="formulario()">
        <div class="row"> 
          <div class="col-md-12 row">
            <input type="hidden" v-model="input_prueba_venta_id">   
  ...
  <div class='card-body'><h1 class=''>Lista de prueba_venta </h1>  
    ...
    <div class='col-md-9 col-sm-12'>
      <b-button v-if="$can('prueba_venta Anadir')"  @click='anadir_registro()' type='button' class='btn btn-wangir btn-lg' data-toggle='button' size='sm' aria-pressed='false' variant='success' style='margin-bottom: 5px; margin: 5px;'>Añadir registro
      </b-button>
    </div>
    <div class='col-md-3 col-sm-12'>
      ...
      <template v-slot:cell(Acciones)='data'>
          ...
          <b-button-group>
            <b-button v-if="$can('prueba_venta Editar')"  size='sm' variant='warning' @click='editar_registro(data.item.id)' type='button' class='btn-sm btn btn-wangir mr-1' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>Editar
            </b-button>
            <b-button v-if="$can('prueba_venta Eliminar')"  v-b-modal.moda-eliminar @click='eliminar_registro(data.item.id)'
                type='button' class='btn-sm btn btn-danger mr-1' size='sm' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>
            ...
            <!--  
            <a v-bind:href=" data.item.id+'/Sucursale'"  class='btn-sm btn btn-success mr-1' size='sm'  style='margin: 0px;height: 15px;'>Surcusales </a>
            -->
          </b-button-group>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      { key: 'id', sortable:true,class:' one-linea'},
      { key: 'fecha_venta', sortable:true,class:' one-linea'},
      { key: 'users_id', sortable:true,class:' one-linea'},
      { key: 'estado_venta_id', sortable:true,class:' one-linea'},
      { key: 'created_at', sortable:true,class:' one-linea'},
      { key: 'updated_at', sortable:true,class:' one-linea'},
      
      ],


      input_prueba_venta_id:[],
  ...
  methods: {
    ...
    consulta(page = 1){
      this.page=page;
      axios.get("prueba_venta/consulta?page=" +page+"&consulta_data="+this.input_consulta_data)
      .then(response => {
    ...
    data_foraneo(){
      axios.get("prueba_venta/create").then(response => {
    ...
    this.input_prueba_venta_id=data_id;
    },
    eliminar_registro_delete(){
      var data_id=this.input_prueba_venta_id;
      axios.delete(`prueba_venta/${this.input_prueba_venta_id}`).then(response => {
    ...
    anadir_registro(){
      this.validacion="";
      this.formulario_prueba_venta=true;
    ...
    formulario(){
      ...
      if(this.editar_dato == true){
        axios.put(`prueba_venta/${this.input_prueba_venta_id}`, data)
        .then(response => {
            ...
            if(response.data.id){
              ...
              this.consulta(this.page);
              this.formulario_prueba_venta=false;
        ...
        axios.post("prueba_venta", data).then(response => {
            const datos = response.data;
            if(response.data.errors){
              this.$toastr.warning("Verifique los datos", "Verifique los datos");
              this.validacion=response.data.errors;
    ...
    editar_registro(data_id){//show 
      ...
      this.mensaje_formulario="Editar un registro"
        axios.get(`prueba_venta/${data_id}`).then(response => {
            const data = response.data;
            if(!response.data){
              ...
              this.input_prueba_venta_id = data.id
